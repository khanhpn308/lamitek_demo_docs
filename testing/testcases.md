...
* **Mô tả:** Xác minh độ trễ < 500ms.
* **Các bước:**
    1. Cấu hình ESP32 chèn Timestamp (T1 - Milliseconds) vào payload khi bắt đầu gửi.
    2. Trên React frontend, ngay khi nhận được gói tin qua Websocket, lấy Timestamp hiện tại (T2).
    3. Tính toán $\Delta T = T2 - T1$.
* **Kết quả mong đợi:** Giá trị trung bình của $\Delta T$ sau 1000 mẫu phải nhỏ hơn 500ms.
...
* **Các bước:**
    ...
    2. Phục hồi mạng.
* **Kết quả mong đợi:** *Trường hợp lý tưởng:* Pi 4 có cơ chế lưu tạm (buffer/QoS) và đẩy bù dữ liệu lên InfluxDB kèm đúng timestamp cũ khi có mạng lại. *Trường hợp tối thiểu:* Hệ thống tự động kết nối lại WebSocket mượt mà và tiếp tục vẽ biểu đồ thời gian thực.